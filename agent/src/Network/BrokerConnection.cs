...
namespace SelfDesk.Agent.Network;
...
public sealed class BrokerConnection : IAsyncDisposable
{
    ...
    public BrokerConnection(AgentConfig cfg, ILogger log)
    {
        _cfg = cfg;
        _log = log;
    }
    ...
    public async Task ConnectAndAuthAsync(CancellationToken ct)
    {
        ...
        {
            caBundle = [X509Certificate2.CreateFromPemFile(_cfg.TlsCaPath)];
        }
        ...
        _log.LogInformation("Autenticado no broker como {AgentId}", _cfg.AgentId);
    }
