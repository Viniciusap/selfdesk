...
# Note the printed SHARED_SECRET — you'll need it on every other machine.
# Copy certs/ca-cert.pem to each Windows machine.
sudo ufw allow from <YOUR_SUBNET>/24 to any port <LISTEN_PORT> proto tcp
node dist/index.js
```

**1. Broker — Windows Server**

```powershell
iwr https://github.com/Viniciusap/selfdesk/releases/latest/download/selfdesk-broker-win-x64.zip -OutFile selfdesk-broker.zip
Expand-Archive selfdesk-broker.zip -DestinationPath selfdesk-broker
cd selfdesk-broker
...
```powershell
iwr https://github.com/Viniciusap/selfdesk/releases/latest/download/selfdesk-agent-win-x64.zip -OutFile selfdesk-agent.zip
Expand-Archive selfdesk-agent.zip -DestinationPath selfdesk-agent
cd selfdesk-agent
# Copy ca-cert.pem from the broker to this folder first, then:
powershell -ExecutionPolicy Bypass -File scripts\bootstrap.ps1 -Role sender
# Prompts for: broker host, SHARED_SECRET, AGENT_ID, encoder.
...
```powershell
iwr https://github.com/Viniciusap/selfdesk/releases/latest/download/selfdesk-viewer-win-x64.zip -OutFile selfdesk-viewer.zip
Expand-Archive selfdesk-viewer.zip -DestinationPath selfdesk-viewer
cd selfdesk-viewer
# Copy ca-cert.pem from the broker to this folder first, then:
...
> **`ca-cert.pem`** is generated by the broker bootstrap. Copy it from `selfdesk-broker/certs/ca-cert.pem` to each Windows machine (into the extracted folder) before running bootstrap on that machine.
